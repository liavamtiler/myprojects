...
productDetails.put("/likeproduct/:pid" ,async (req,res)=>{
  const findMatch= (await client.query( `SELECT * from interaction where product_id=${req.params.pid}`)) 
  if (findMatch.rows.length > 0 ){ 
    const createProductLike = (await client.query(`UPDATE interaction SET like_number = like_number+1 where product_id=${req.params.pid} `)).rows[0] 
    console.log(`length> 1 ${createProductLike}`)

   }
   else{ const Insert = (await client.query(`INSERT INTO interaction (product_id, like_number) VALUES ($1,$2)`,[
    req.params.pid,
    1
])).rows[0]
...
productDetails.get("/alllikeproduct/:pid" ,async (req,res)=>{
  const totalLike =  (await client.query(`SELECT * from interaction where product_id=${req.params.pid}`)).rows
...
productDetails.post("/comments/:cid",async (req,res)=>{
  ...
  const insertedComment = await client.query(`INSERT INTO comments (content,created_at,user_id,product_id,user_name) VALUES ($1,$2,$3,$4,$5)`,[
...
comment.get("/comments/:cid", async (req,res) => {
  const productComment = (await client.query(`SELECT * FROM comments WHERE product_id = ${req.params.cid} ORDER BY created_at DESC`)).rows
...
comment.put("/comments", async (req,res) => {
  ...
  const editedComment = ((await client.query('UPDATE comments SET content = $1 WHERE id = $2', [content,id])).rows[0]);
...
comment.get("/check/",async (req,res)=>{
  const checkComment = (await client.query(`SELECT * FROM comments WHERE user_id = ${req.session["user"].id} `)).rows[0]
