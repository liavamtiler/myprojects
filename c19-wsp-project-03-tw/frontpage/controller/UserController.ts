...
export class UserController {
    ...
    userHistory =  async(req:Request,res:Response)=>{
      ...
      for (const products of cartProducts){ 
        ...
        await this.ProductService.insertToHistory(products.product_name,
    ...
    giveComment = async(req:Request,res:Response)=>{
      ...
      const insertedComment = await this.userService.giveCommentToProduct(content,new Date(),req.session["user"].id,productID,req.session["user"].name)    
    ...
    likeProduct = async(req:Request,res:Response)=>{
        let producID = parseInt(req.params.pid)
        
        const findMatch= await this.ProductService.getProductfromLike(producID);


        console.log(findMatch)
         
        if (findMatch.length === 0 || findMatch ==null ){ 
          const initlike = await this.userService.initLike(producID);
          res.status(200).json(initlike)
          return 
         }
        
        const createProductLike = await this.userService.givelike(producID);
      ...
      cancelLike =async(req:Request,res:Response)=>{
        ...
        if (findMatch.length > 0 ){ 
          const cancelLike = await this.userService.cancelLike(producID)
